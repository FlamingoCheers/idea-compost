...
### 轮 4 · 魔鬼代言人（仅深模式：方法菌独立反方轮）

```text
—— 发酵轮 4：最强反方论证（魔鬼代言人） ——

你能看到轮 1 + 轮 2 + 轮 3 的全部输出。现在暂时放下「帮用户」的立场，
专职扮演魔鬼代言人：假设目前所有正在形成的判断都是错的，为反方构建
最强论证。

要求：
- 对轮 3 中置信度最高的 1-2 个新假设，各构建一条「钢人级」反驳
  （最强形式，禁止稻草人）。
- 指出这批思考可能整体走错的方向：如果结论错了，最可能错在哪一步？
- 可以推翻自己此前轮次的观点（这是本轮的职责，不是背叛）。

输出严格 JSON：
{
  "agent": "{{agent_name}}",
  "strongest_objections": [{"against": ["{{CODE}}9"], "text": "…"}],
  "failure_modes": ["如果这批思考整体错了，最可能的错处"],
  "claims": [{"id": "{{CODE}}12", "text": "…", "confidence": "…"}],
  "citations": ["claim:{{CODE}}9"]
}
```

> 停止条件由代码层判定（03 §7：信息饱和/冲突收敛/4 轮上限/复读检测），不进入 Prompt。浅模式 = 轮1+轮3；深模式 = 标准 3 轮 + 独立魔鬼代言人轮（03 §6.1）。
